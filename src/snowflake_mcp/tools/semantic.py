...
"""
Semantic view tools for Snowflake MCP Server.
"""
...
def list_semantic_views(
    ...
    try:
        result = execute_query(statement, connection_manager)
...
def describe_semantic_view(
    ...
    try:
        ...
        raise SnowflakeException(tool="describe_semantic_view", message=e)
...
def get_semantic_view_ddl(
    ...
    try:
        return execute_query(statement, connection_manager)[0].get("DDL")
    except Exception as e:
...
def query_semantic_view(
    ...
    try:
        ...
        return execute_query(statement, connection_manager)
    except Exception as e:
...
def initialize_semantic_manager_tools(server: FastMCP, connection_manager: SnowflakeConnectionManager):
    """Initialize semantic view tools for the MCP server."""
    ...
    def list_semantic_views_tool(
        ...
        return list_semantic_views(
            connection_manager, database_name, schema_name, like, starts_with
        )
    ...
    def describe_semantic_view_tool(
        ...
        return describe_semantic_view(
            connection_manager, view_name, database_name, schema_name
        )
